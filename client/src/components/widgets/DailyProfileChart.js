...
class DailyProfileChart extends Component {
  ...
  state = { traffic: true, pattern: true };
  ...
  makeCurveFunctions() {
    ...
    this.xAxis = d3.axisBottom(this.scaleX).ticks(24);

    this.yAxis = d3.axisLeft(this.scaleY).ticks(5);

    this.scaleY2 = d3.scaleLinear().range([this.chartHeight, 0]);

    this.yAxis2 = d3.axisRight(this.scaleY2).ticks(5);
  ...
  drawChart() {
    if (this.props.dailyProfile == null || this.props.dailyPattern == null) return;
    ...
    svg
      .append('g')
      .attr('transform', `translate(${this.svgWidth - this.margin.right},${this.margin.top})`)
  ...
  updateChart() {
    ...
    if (!this.svg) this.drawChart();

    this.dates = this.props.dates;

    let maxData = d3.max(this.props.dailyProfile.tickets, d => d) || 1000;
    maxData = 40;
    this.scaleY.domain([0, maxData]);

    let maxTraffic = d3.max(this.props.dailyProfile.traffic, d => parseFloat(d)) || 1000;
    maxTraffic = 500;
    this.scaleY2.domain([0, maxTraffic]);
  ...
  updateRealPath() {
    ...
    if (this.props.dailyPattern) {
      pattern = this.props.dailyPattern.stats.profile.map(d => {
        return { upper: d[1].median + d[1].iqr * 1.5, median: d[1].median, lower: d[1].median - d[1].iqr * 1.5 };
      });
    ...
    if (this.props.dailyProfile) {
      let traffic = this.props.dailyProfile.traffic.map(d => parseFloat(d));

      let secondary = this.svg
        .select('g.secondary')
        .selectAll('.secondary-line')
        .data([traffic]);
  ...
  toggleTraffic() {
    ...
    this.setState({ ...this.state, traffic: traffic });
  }
  ...
  render() {
    return (
      <div className={`DailyProfileChart traffic-${this.state.traffic} pattern-${this.state.pattern}`}>
        {this.renderProps()}
        <div className="d3-chart" id={this.props.id} ref={divElement => (this.divElement = divElement)} />
        <div>
          <button onClick={() => this.toggleTraffic()}>TRAFFIC</button>
          <button onClick={() => this.togglePattern()}>PATTERN</button>
        </div>
      </div>
    );
  }
